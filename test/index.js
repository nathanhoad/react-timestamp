...
describe('Timestamp', () => {
  it('renders a normal time ago', done => {
    let timestamp = TestUtils.renderIntoDocument(<Timestamp time={Moment().subtract(3, 'days')} />);
    Should(ReactDOM.findDOMNode(timestamp).textContent).equal('3 days ago');

    timestamp = TestUtils.renderIntoDocument(<Timestamp time={Moment().subtract(3, 'months')} />);
  ...
  it('renders a normal time in the future', done => {
    ...
    timestamp = TestUtils.renderIntoDocument(<Timestamp time={Moment().add(4, 'months')} />);
  ...
  describe('renders a normal time in local time', () => {
    it('in full', done => {
      ...
      let timestamp = TestUtils.renderIntoDocument(<Timestamp time={utc} format="full" />);

      Should(ReactDOM.findDOMNode(timestamp).textContent).equal(local.format('D MMM YYYY, h:mma'));

      timestamp = TestUtils.renderIntoDocument(<Timestamp time={utc.format('YYYY-MM-DD H:mm:ss')} format="full" />);
      ...
      timestamp = TestUtils.renderIntoDocument(<Timestamp time={utc} format="full" includeDay />);
    ...
    it('just the date', done => {
      ...
      let timestamp = TestUtils.renderIntoDocument(<Timestamp time={utc} format="date" />);
    ...
    it('includes the day', done => {
      ...
      let timestamp = TestUtils.renderIntoDocument(<Timestamp time={utc} format="date" includeDay />);
  ...
  it('renders an integer timestamp in local time', done => {
    ...
    let timestamp = TestUtils.renderIntoDocument(<Timestamp time={utc.unix()} format="full" />);
  ...
  it('renders "never" for a bad date', done => {
    let timestamp = TestUtils.renderIntoDocument(<Timestamp time="not a date or time" format="full" />);

    Should(ReactDOM.findDOMNode(timestamp).textContent).equal('never');

    timestamp = TestUtils.renderIntoDocument(<Timestamp time={NaN} format="full" />);

    Should(ReactDOM.findDOMNode(timestamp).textContent).equal('never');

    timestamp = TestUtils.renderIntoDocument(<Timestamp time={null} format="full" />);
  ...
  it('renders "never" for a null date', done => {
    let timestamp = TestUtils.renderIntoDocument(<Timestamp time={null} format="full" />);
  ...
  it('renders without converting from UTC', done => {
    ...
    let timestamp = TestUtils.renderIntoDocument(<Timestamp time={sydney.format()} utc={false} format="full" />);
    ...
    timestamp = TestUtils.renderIntoDocument(<Timestamp time={perth.format()} utc={false} format="full" />);
    ...
    timestamp = TestUtils.renderIntoDocument(<Timestamp time={la.format()} utc={false} format="full" />);

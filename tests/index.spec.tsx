...
export class TestTimestamp {
  ...
  public testWithDay(date: Date, expectedOutput: string) {
    const { root } = TestRenderer.create(
      <Timestamp date={date} options={{ includeDay: true }} />
    );
    Expect(root.findByType("time").children).toEqual([expectedOutput]);
  }
  ...
  public testJustDate(date: Date, expectedOutput: string) {
    const { root } = TestRenderer.create(
      <Timestamp date={date} options={{ format: "date" }} />
    );
    Expect(root.findByType("time").children).toEqual([expectedOutput]);
  }
  ...
  public testJustTime(date: Date, expectedOutput: string) {
    const { root } = TestRenderer.create(
      <Timestamp date={date} options={{ format: "time" }} />
    );
    Expect(root.findByType("time").children).toEqual([expectedOutput]);
  }
  ...
  @TestCase(
    new Date(2022, 3, 28, 10, 30),
    new Date(2022, 3, 28, 10, 31),
    "1 minute"
  )
  @TestCase(
    new Date(2022, 3, 28, 10, 30),
    new Date(2022, 3, 26, 10, 30),
    "2 days"
  )
  @TestCase(
    new Date(2022, 3, 28, 10, 30),
    new Date(2024, 3, 26, 10, 31),
    "2 years"
  )
  public testDistanceBetweenDates(
    date: Date,
    relativeTo: Date,
    expectedOutput: string
  ) {
    const { root } = TestRenderer.create(
      <Timestamp date={date} relativeTo={relativeTo} />
    );
    Expect(root.findByType("time").children).toEqual([expectedOutput]);
  }
}
